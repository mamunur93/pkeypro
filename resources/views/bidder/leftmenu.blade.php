                ...
                <!--Dashboard-->
                  ...
                  <li class="{{ bidderActive($active_class,'user_management')}}">
                    ...
                    <ul class="submenu">
                      ...
                      <li class="{{ $request->segment(2) == 'business' ? 'active active-sub' : '' }}"><a href="{{URL_BUSINESS_EDIT}}/{{$user->slug}}" title="Business">{{getPhrase('Business Details')}}</a></li>

                      <li class="{{ $request->segment(2) == 'org' ? 'active active-sub' : '' }}"><a href="{{URL_ORG_EDIT}}/{{$user->slug}}" title="Org">{{getPhrase('Organization Details')}}</a></li>

                      <li class="{{ $request->segment(2) == 'bank' ? 'active active-sub' : '' }}"><a href="{{URL_BANK_EDIT}}/{{$user->slug}}" title="Bank">{{getPhrase('Bank Details')}}</a></li>

                      <li class="{{ $request->segment(2) == 'certificate' ? 'active active-sub' : '' }}"><a href="{{URL_CERTIFICATE_EDIT}}/{{$user->slug}}" title="Certificate">{{getPhrase('Certificates and & Membership')}}</a></li>
  
                    @if($user->role_id !=4 )
                    <li class="{{ $request->segment(2) == 'slave' ? 'active active-sub' : '' }}"><a href="{{URL_SLAVE_EDIT}}/{{$user->slug}}" title="Org">{{getPhrase('Slave USers')}}</a></li>
                    @endif
                  ...
                  <li class="{{ bidderActive($active_class,'auctions')}}">
                    ...
                    <ul class="submenu">
                      ...
                      <!-- <li class="{{ $request->segment(2) == 'create-auctions' ? 'active active-sub' : '' }}"><a href="{{ URL_LIST_AUCTIONS }}" title="Create Auctions">  {{getPhrase('create auctions')}}  </a></li> -->
