...
class AuctionsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(!checkRole(getUserGrade(7)))
        {
         ...
         $columns = array(
         'title'            => 'bail|required|unique:auctions|max:100',
      ...
      /*  $record->listing_type       = getSetting('listing_type','auction_settings');
        ...
        // CUSTOMER ADDED BY ISTIYAK
        $record->bidininame = $request->bidininame;
        $record->currency = $request->currency;
        $record->bit_event_category = $request->bit_event_category;
        $record->quantity = $request->quantity;
        $record->unit_of_measurement = $request->unit_of_measurement;
        $record->unit_price = $request->unit_price;
        $record->total_price = $request->total_price;
        
        $record->line_item1 = $request->line_item1;
        $record->line_item2 = $request->line_item2;
        $record->line_item3 = $request->line_item3;
        $record->line_item4 = $request->line_item4;
        $record->line_item5 = $request->line_item5;
        if(!env('DEMO_MODE')) {

            if($request->hasFile('file'))
            {
                $record->file = $this->processUpload($request,$record);
            }

        }
        // CODE ENDED BY ISTIYAK
        ...
        */
        ...
        if (!env('DEMO_MODE')) {
            if ($request->hasFile('file')) {
                ...
                $record->file = $this->processUpload($request,$record);
                if ($old_file!='') {
                    $this->deleteFile($old_file, AUCTION_IMAGES_PATH);
                    $this->deleteFile($old_file, AUCTION_IMAGES_THUMBPATH);
                }
            }
        }
        // CODE ENDED BY ISTIYAK
    ...
    /**
     ...
     public function getEventCategories(Request $request)
     {
        ...
        $sub_categories = \App\User::select('id','username')->get();
        // return "halt by istiyak";
        return json_encode(array('close_users'=>$sub_categories));
     }
